...
public class RestaurantApprovalOutboxScheduler implements OutboxScheduler {
    ...
    @Transactional
    @Scheduled(
            fixedRateString = "${order-service.outbox-scheduler-fixed-rate}",
            initialDelayString = "${order-service.outbox-scheduler-initial-delay}"
    )
    public void processOutboxMessage() {

        Optional<List<OrderApprovalOutboxMessage>> outboxMessagesResponse = approvalOutboxHelper.getOrderApprovalOutboxMessageByOutboxStatusAndSagaStatus(
