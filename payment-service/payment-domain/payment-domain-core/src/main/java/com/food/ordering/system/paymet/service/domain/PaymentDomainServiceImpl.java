...
import com.food.ordering.system.paymet.service.domain.valueobject.CreditHistoryId;
import com.food.ordering.system.paymet.service.domain.valueobject.TransactionType;
...
public class PaymentDomainServiceImpl implements PaymentDomainService{
    ...
    public PaymentEvent validateAndCancelPayment(Payment payment, CreditEntry creditEntry, List<CreditHistory> creditHistories, List<String> failureMessages, DomainEventPublisher<PaymentCancelledEvent> paymentCancelledEventDomainEventPublisher, DomainEventPublisher<PaymentFailedEvent> paymentFailedEventDomainEventPublisher) {
        ...
        if (failureMessages.isEmpty()){
            ...
            log.error("Payment cancellation is failed for order with id  : {} ", payment.getOrderId().getValue());
            payment.updateStatus(PaymentStatus.FAILED);
            return new PaymentFailedEvent(payment,ZonedDateTime.now(ZoneId.of(UTC)),failureMessages,paymentFailedEventDomainEventPublisher);
        }
    }
    ...
    private void validateCreditEntry(Payment payment, CreditEntry creditEntry, List<String> failureMessages) {
        ...
        if (payment.getPrice().isGreaterThan(creditEntry.getTotalCreditAmount())){
            log.error("Customer with id : {} , dosen`t have enough credit for payment", payment.getCustomerId().getValue());
            failureMessages.add("customer with id: = " + payment.getCustomerId().getValue() + " dosen`t enough credit for payment");
    ...
    private void validateCreditHistory(CreditEntry creditEntry, List<CreditHistory> creditHistories, List<String> failureMessages) {
        ...
        if (totalDebitHistory.isGreaterThan(totalCreditHistory)){
            log.error("Customer with id : {} , dosen`t have enough credit according to credit history ", creditEntry.getCustomerId().getValue());
            failureMessages.add("customer with id: = " + creditEntry.getCustomerId().getValue() + " dosen`t have enough credit according to credit history for payment");
